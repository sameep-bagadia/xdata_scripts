...
def main():
	...
	trip_lengths = np.ones(num_days)
	trip_times = np.ones(num_days)
	trip_speed = np.ones(num_days)
	passenger_counts = np.ones(num_days)
	trip_counts = np.ones(num_days)

	agg_trip_lengths = np.ones(7)
	agg_trip_times = np.ones(7)
	agg_trip_speed = np.ones(7)
	agg_passenger_counts = np.ones(7)
	agg_trip_counts = np.ones(7)
		...
		for filename in f:
			...
			with open(filepath) as nycf:
				for line in nycf:
					...
					if (haversine(drop_lon, drop_lat, event_lon, event_lat) < 0.5):
	...
	pc_trip_lengths = np.ones(num_days)
	pc_trip_times = np.ones(num_days)
	pc_trip_speed = np.ones(num_days)
	pc_passenger_counts = np.ones(num_days)
	pc_trip_counts = np.ones(num_days)
	...
	wow_trip_lengths = np.ones(num_days)
	wow_trip_times = np.ones(num_days)
	wow_trip_speed = np.ones(num_days)
	wow_passenger_counts = np.ones(num_days)
	wow_trip_counts = np.ones(num_days)
