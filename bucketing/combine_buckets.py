...
def main():
	with open('/lfs/local/0/sameepb/xdata/nyc/bucket_agg/bucket_agg2.csv') as nycf:
		for line in nycf:
			line = line[:-1]
			line_arr = line.split(',')
			...
			#insert(nbx + space_gran, nby + space_gran, nbt + 1, line_arr)  # overlap: the designated bucket number is the central value
	outfile = '/lfs/local/0/sameepb/xdata/nyc/bucket_agg/bucket_combined2.csv'
	counts = []
