...
def main():
	with open('/lfs/local/0/sameepb/xdata/nyc/bucket_agg/bucket_combined2_dropoff.csv') as nycf:
		for line in nycf:
			line = line[:-1]
			line_arr = line.split(',')
	...
	filepath = '/lfs/local/0/sameepb/xdata/nyc/events3_dropoff/'
	...
	for key in bmap.keys():
		...
		if (True):
			...
			pc_dist = abs(event_trip_dist - past_trip_dist) / math.sqrt(dist_var)
			pc_time = abs(event_trip_time - past_trip_time) / math.sqrt(time_var)
			pc_speed = abs(event_trip_speed - past_trip_speed) / math.sqrt(speed_var)
			pc_pc = abs(event_pass_cnt - past_pass_cnt) / math.sqrt(pc_var)
			pc_cnt = abs(event_trip_cnt - past_trip_cnt) / math.sqrt(cnt_var)

			(lon, lat) = GetLongLat(bx, by)
			(date, time) = GetDateTime(bt)

			'''
			date \t time \t latitude \t longitude \t change / std \t metric value \t avg metric value in past 4 weeks \t sd of metric value in past 4 weeks \t volume of trips \t volume of trips in last 4 weeks \t bucketx \t buckety \t buckett
			'''
