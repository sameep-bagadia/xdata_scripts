# This file takes in data from 'data_input' file and writes the same data to 'dataf' after filtering those buckets which are in the first four (lookback) weeks. We only keep those buckets for which we have the entire previous data of 4 (lookback) weeks. We also convert the date and time to time bucket bt.
